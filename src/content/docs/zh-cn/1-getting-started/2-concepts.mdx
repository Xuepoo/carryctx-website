...
Agent 可以带有可选的 `kind` 元数据（`commander` 或 `subagent`），在注册时用 `--kind` 指定。它用于识别角色，但不是权限边界。

## Team（团队）

Team 是一份持久化的、项目级的 agent 名册，可以有一位指挥官（commander）。因为它和任务存放在共享状态数据库中，所以跨越窗口、会话和关联 Worktree 仍然存在。
...
任务还可以带有 `team_id`（关联的团队）和 `required_role`（建议性的角色标签）。两者都不参与上面的状态机。
