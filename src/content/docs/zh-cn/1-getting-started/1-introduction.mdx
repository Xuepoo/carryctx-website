...
CarryCtx 是 AI 编码 Agent 的意图与管理层。它持久保存团队的**意图**——带优先级与工作流、被依赖关系门控的任务；**分工**——由一位指挥官与角色化子 Agent 组成的持久团队，交接由状态机强制推进；以及**发生过的事实**——只追加的事件流、具备 Git 感知的检查点、不可变的决策记录、全文搜索。然后，它通过一个纯 CLI 或一台 MCP 服务，把某位调用方恰好需要的那一片上下文交出去。
...
具体地说，0.7.0 不包含调度器、不包含 worker 运行时、不包含 lease 或心跳机制、不包含 prompt 缓存，也不包含 token 优化器。它提供的是这些 harness 协同时所依赖的持久基底：被依赖门控的就绪队列、通过 CAS 更新在并发下安全仲裁的认领与交接、键集分页的事件流，以及只读的团队投影。`carryctx team context` 负责把团队的准确画面交给指挥官；拿到这幅画面之后怎么决策、以及真正把工作派发出去，都在 CLI 之外。CarryCtx 不运行任何人的 agent。
