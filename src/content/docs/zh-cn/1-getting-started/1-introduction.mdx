...
CarryCtx 0.11.4 不声称提供通用 Automation Engine 或 Completion Gates。它提供的是完整生命周期所需的持久基底：依赖门控任务、CAS 安全的认领与交接、有界 MCP 调用、显式清理策略、`task edit --force` 修正、键集分页事件和只读团队投影。对于 jj colocated 仓库，在 Git Worktree 或 hook 不安全的地方会主动阻止操作。CarryCtx 不调度也不运行任何人的 agent。
