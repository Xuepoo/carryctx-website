...
本文完整走一遍生命周期：建立项目契约、规划依赖、注册角色、创建任务、在 Worktree 中运行会话、记录进度、交接评审、协调清理，并恢复上下文。请在一个已有的 Git 仓库中执行以下步骤。
...
已经处于 `completed` 或 `cancelled` 的任务不能普通编辑。有效的负责人可以使用 `carryctx task edit CTX-0001 --title "修正后的标题" --force` 做更正；这次更正会记录在审计日志中。
...
评审完成、任务结束后，v0.11.0（自 v0.8 起）可以根据项目策略创建持久化清理请求。使用 `carryctx worktree cleanup list` 查看待处理或阻塞请求，用 `carryctx worktree cleanup run` 重试安全协调。路径已经不存在时，会幂等地按“已移除”处理，并清理对应的陈旧注册；脏 Worktree、活跃会话、锁、缺失 Git 元数据与 jj colocated 仓库仍会作为可见阻塞。未知注册属于查找错误，不应当按已移除的工作区处理。使用 `carryctx stats` 与 `carryctx event list` 为发布保留审计和统计证据。

## 10. 合并来自另一个 clone 的状态

项目状态既可移植，也可合并。当同一个项目存在于多个 clone 中时，从其中一个导出快照，再合并进另一个——这是针对行的语义化三方合并，而不是对 JSONL 做逐行合并：

```bash
carryctx export --pack-format dir -o ./pack --snapshot
# 由你自己把 ./pack（或快照 ref）搬到另一个 clone
carryctx import ./pack --mode merge
```

若存在阻断冲突，会暂存以待复核：用 `carryctx conflict list` 查看，再用 `carryctx conflict resolve` 与 `carryctx conflict apply` 逐个了结。不支持对快照提交执行原生 `git merge`。完整工作流、快照 ref 与退出码见[合并与快照](/zh-cn/2-cli-reference/11-merge-and-snapshots/)。
