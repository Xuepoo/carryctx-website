...
评审完成、任务结束后，v0.8 可以根据项目策略创建持久化清理请求。使用 `carryctx worktree cleanup list` 查看待处理或阻塞请求，用 `carryctx worktree cleanup run` 重试安全协调。脏 Worktree、活跃会话、锁、缺失路径与 jj colocated 仓库会保留为可见阻塞。使用 `carryctx stats` 与 `carryctx event list` 为发布保留审计和统计证据。
