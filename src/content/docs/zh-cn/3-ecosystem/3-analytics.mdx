...
在 v0.11.0（自 v0.8 起）中，`sync pull` 会在替换任何内容之前完整校验本地目录中的数据库，并拒绝属于其他项目的数据库：
...
`sync` 复制整个数据库文件；`export`/`import`（0.11.0，自 0.8.2 起）则更进一步，产出可移植目录（`manifest.json` 加每表 `*.jsonl`），在机器之间换路径也不怕——导入到全新检出时会重锚定绝对路径，而不是把旧路径带过去。传输方式仍然由你定（`scp`、共享文件夹、Syncthing）；CarryCtx 只负责序列化、校验与原子换入。从 `carryctx export --help` 开始。
