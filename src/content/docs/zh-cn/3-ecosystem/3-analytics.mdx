---
title: 分析、Hook 与本地文件同步
---

import { Aside } from '@astrojs/starlight/components';

当任务、检查点和图谱都进入日常使用之后，还有三项本地能力补齐了整套生态：`carryctx stats` 用于报告实际的工作进展，`carryctx hooks` 用于把 CarryCtx 自动接入 Git，`carryctx sync` 则用于（完全可选地）把项目状态复制到你自己指定的本地目录，或从该目录复制回来。CarryCtx 记录生命周期状态与证据；外部 harness 仍负责拉起 agent、调度进程、选择模型和运行验证。
...
这正是 `use-carryctx` 技能建议「导出项目统计报告用于 PR 描述或文档」时所指的内容：直接用 `--markdown --output` 生成一份，粘进 PR 正文或周报，而不必手写。
...
<Aside type="note">
自 0.11.2 起，本地无状态时文本模式 `carryctx stats` 会在仓库带有 in-repo 发布 ref（`refs/heads/carryctx-snapshots`，clone 后通常为 `origin/carryctx-snapshots`）时打印恢复提示——拉取后执行 `carryctx import --from-git <ref> --mode replace` 即可。该提示本身不改变任何状态。
</Aside>
...
`sync push` 会把项目 SQLite 数据库经过 checkpoint 的 `VACUUM INTO` 快照发布到 `--remote` 指定的本地目录下（必填，无默认值——请选择你可控的路径）；`sync pull` 则把它取回本地项目数据库所在位置。整个机制仅此而已：对着你指定的目录做一次普通的本地文件复制。没有网络协议、没有云服务、没有服务端、没有定时同步，二进制里根本不含任何网络栈——`--remote` 是文件系统路径，不是 URL。

在 v0.11.4（自 v0.8 起）中，`sync pull` 会在替换任何内容之前完整校验本地目录中的数据库，并拒绝属于其他项目的数据库：
...
<Aside type="caution">
`sync` 从不会被任何其他命令自动调用，它本身也不跨机器、不走网络：它只是往一个本地路径写文件，再从一个本地路径读文件。如果那个路径恰好是挂载的网络共享、外部硬盘，或者由别的工具替你同步的文件夹，那么搬运字节的是那个挂载点或那个工具，而不是 CarryCtx。没有云端账号、默认远端或后台进程；只要你从不运行 `carryctx sync`，项目数据就永远不会离开这台机器。
</Aside>
...
`sync` 复制整个数据库文件；`export`/`import`（0.11.4，自 0.8.2 起）则更进一步，产出可移植目录（`manifest.json` 加每表 `*.jsonl`），在机器之间换路径也不怕——导入到全新检出时会重锚定绝对路径，而不是把旧路径带过去。传输方式仍然由你定（`scp`、共享文件夹、Syncthing）；CarryCtx 只负责序列化、校验与原子换入。从 `carryctx export --help` 开始。
