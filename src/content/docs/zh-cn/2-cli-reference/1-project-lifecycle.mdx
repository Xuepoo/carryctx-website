...
v0.9.0（自 v0.8 起）在应用迁移前会在状态目录的 `backups/` 下创建并校验 `VACUUM INTO` 备份，并验证迁移历史是连续的前缀后才继续。
...
可移植的离线优先项目交换格式(0.9.0，自 0.8.2 起)。`backup`/`restore` 在一台机器上搬运原生 SQLite 快照,而 `export` 把项目状态序列化为目录结构——`manifest.json`、`project.json` 加每表一个 `*.jsonl` 文件,由你自己带到另一台机器(U 盘、`scp`、Syncthing、NAS 均可;CarryCtx 绝不触碰网络):
...
导入到全新的 Git 检出中会复用包内的项目身份,并把绝对路径重锚定到新位置。在已初始化的项目上不带 `--mode` 重复导入会被拒绝(`STATE_CONFLICT`);`--mode merge` 尚未实现,会报告 `UNSUPPORTED_OPERATION`。两个方向都支持 `--dry-run`。
