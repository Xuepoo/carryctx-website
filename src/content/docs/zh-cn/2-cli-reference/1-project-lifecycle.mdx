...
本页覆盖完整项目生命周期：建立项目契约、规划依赖、协调角色、运行会话与 Worktree、记录进度和证据、交接评审、协调清理，以及审计发布结果。前提是已有 Git 仓库。
...
诊断并可自动修复项目 SQLite 状态数据库中潜在的问题。它会检查:全局配置的有效性、Git 仓库发现情况、已安装的 CarryCtx git hook(`post-commit`、`prepare-commit-msg`)、[Jujutsu (jj) colocation](https://jj-vcs.dev/) 检测（这是针对实时 Worktree 创建、清理和 hook 的操作性故障即拒绝执行防护，而不只是提示信息）、数据库连接与 schema 版本、孤立任务(owner agent 已不存在的任务)、进行中的任务,以及活跃会话。
...
v0.10.0（自 v0.8 起）在应用迁移前会在状态目录的 `backups/` 下创建并校验 `VACUUM INTO` 备份，并验证迁移历史是连续的前缀后才继续。
...
`doctor` 会检测**过期的 Worktree 注册记录**。检测是只读的；清理绝不会隐式发生，必须同时给出两个参数:
...
## `carryctx export` / `carryctx import`

可移植的离线优先项目交换格式(ctxpack 目录；0.8.2 引入，`ctxpack` v2 与 `--mode merge` 自 0.10.0 起现行)。`backup`/`restore` 在一台机器上搬运原生 SQLite 快照,而 `export` 把项目状态序列化为目录结构——`manifest.json`、`project.json` 加每表一个 `*.jsonl` 文件,由你自己带到另一台机器(U 盘、`scp`、Syncthing、NAS 均可;CarryCtx 绝不触碰网络):
...
导入到全新的 Git 检出中会复用包内的项目身份,并把绝对路径重锚定到新位置。在已初始化的项目上不带 `--mode` 重复导入会被拒绝(`STATE_CONFLICT`);`--mode merge` 会基于导出 DAG 执行语义化三方合并,并把阻断冲突暂存给 `conflict` 命令族处理。`--snapshot` 会把校验通过的包提交到本地专用 `refs/carryctx/local` ref,`--from-git <ref>` 则导入从本地 ref tip 物化出的包——两者都完全离线。两个方向都支持 `--dry-run`。快照 ref 布局与完整的合并工作流见 [合并与快照](/zh-cn/2-cli-reference/11-merge-and-snapshots/)。
...
| 子命令 | 说明 |
| --- | --- |
| `register --name <NAME> [--provider <P>] [--role <ROLE>] [--kind commander\|subagent]` | 注册一个新 agent,或将已存在的 agent 同步到项目状态中。`--kind` 是描述执行类型的可空元数据。 |
