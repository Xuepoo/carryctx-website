...
`task edit` 只修改传入的 `--title`、`--priority`、`--description` 和 `--required-role`。终态任务被冻结，只有有效负责人显式使用 `--force` 才能更正，并会追加 `task.corrected` 审计事件。它没有用于修改 `--assignee` 或 `--status` 的参数，负责人和状态请使用专门的转换命令。团队关联有自己的子命令：
