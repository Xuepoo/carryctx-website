...
Team 是一份持久化的、项目级的 Agent 名册。它和任务存放在共享的 `state.sqlite` 中，因此跨越会话、窗口与关联 Worktree 仍然存在。

Team 回答了三个以前只存在于 prompt 里、或者只存在于某个人脑子里的问题：**这个团队里有谁**、**每个成员正在做什么**、以及**某个成员此刻需要知道什么**。它不负责运行任何人的 agent。

<Aside type="caution">
CarryCtx 是生命周期管理与持久化层，不是进程调度器。拉起进程、分派工作、重试、并发上限和模型选择全部属于外部 harness。CarryCtx 0.10.0 不声称提供通用 Automation Engine、Completion Gates 或 lease/心跳运行时。
</Aside>
...
`--kind` 接受 `commander` 或 `subagent`，且可为空。它是给 harness 和阅读 `team status` 的人看的提示，不是权限边界。
...
`--format text`（默认）为每个团队输出一行紧凑摘要；`--format markdown` 把同样的数据渲染为 GFM 表格。需要下面展示的 envelope 时使用 `--json`。
...
`--format text`（默认）输出一两行摘要；`--format markdown` 把该投影渲染为 GFM 表格（成员表，以及存在时的任务、阻塞项与近期事件表）。`--json` 输出下面展示的 envelope。
...
|`scopes` / `scope_conflicts`|任务记录的文件范围元数据与冲突投影。|
