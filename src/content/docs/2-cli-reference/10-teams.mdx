...
A Team is a durable, project-scoped roster of agents. It lives in the shared `state.sqlite` as your tasks, which means it outlives sessions, windows, and linked worktrees.
...
<Aside type="caution">
CarryCtx is the lifecycle management and persistence layer, not a process scheduler. Spawning processes, routing work, retries, concurrency limits, and model selection belong to the external harness. CarryCtx 0.10.0 does not claim a generic Automation Engine, Completion Gates, or lease/heartbeat runtime.
</Aside>
...
`--kind` accepts `commander` or `subagent` and is nullable. It is a hint for the harness and for reading `team status`, not a permission boundary.
...
`--format text` (the default) prints a compact one-line summary per team; `--format markdown` renders the same data as a GFM table. Use `--json` for the envelope shown below.
...
`--format text` (the default) prints a one/two-line summary; `--format markdown` renders the projection as GFM tables (members, plus tasks, blockers, and recent events when present). `--json` emits the envelope shown below.
...
|`scopes` / `scope_conflicts`|File-scope metadata and conflict projections when recorded for a task.|
