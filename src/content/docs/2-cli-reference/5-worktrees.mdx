...
<Aside type="note">
v0.8 adds persistent cleanup requests and policies. `worktree cleanup list`, `show`, and `run` reconcile safe removal attempts; dirty worktrees, active sessions, locks, missing paths, and jj-colocated repositories remain observable blockers rather than being silently deleted. `unbind` still only detaches a task.
</Aside>
