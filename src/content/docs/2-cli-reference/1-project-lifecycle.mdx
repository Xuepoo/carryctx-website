...
Before applying migrations, v0.8 creates and verifies a `VACUUM INTO` backup under the state directory's `backups/`; it also validates contiguous migration history before continuing.
...
Portable, offline-first project exchange (0.8.2+). While `backup`/`restore` move a raw SQLite snapshot on one machine, `export` serializes project state into a directory layout — `manifest.json`, `project.json`, and one `*.jsonl` file per table — that you carry to another machine yourself (USB stick, `scp`, Syncthing, NAS; CarryCtx never touches the network):
