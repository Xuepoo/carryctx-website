...
Before applying migrations, v0.11.0 (since v0.8) creates and verifies a `VACUUM INTO` backup under the state directory's `backups/`; it also validates contiguous migration history before continuing.
