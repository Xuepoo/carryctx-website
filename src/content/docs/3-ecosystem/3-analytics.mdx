---
title: Analytics, Hooks & Local File Sync
---
...
This is the same content the `use-carryctx` skill points agents at when it suggests exporting project stats for a PR description or documentation: drop `--markdown --output` into a PR body or a weekly project update instead of writing it by hand.
...
<Aside type="note">
Since 0.11.2, text-mode `carryctx stats` on an empty local state prints a restore hint when the repository carries an in-repo publication ref (`refs/heads/carryctx-snapshots`, usually `origin/carryctx-snapshots` after a clone) — fetch it and run `carryctx import --from-git <ref> --mode replace`. The hint changes nothing on its own.
</Aside>
