...
<Aside type="note">
As of 0.7.0 there is no dedicated MCP tool for the [`team`](/2-cli-reference/10-teams/) command family. Team membership and the `team status` / `team context` projections are CLI-only for now.
</Aside>
