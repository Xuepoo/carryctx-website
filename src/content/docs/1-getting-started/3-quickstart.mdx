...
Blocking conflicts, if any, stage for review: inspect them with `carryctx conflict list`, then settle each with `carryctx conflict resolve` and `carryctx conflict apply`. Native `git merge` of snapshot commits is not supported. The full workflow, snapshot refs, and exit codes are in [Merge & Snapshots](/2-cli-reference/11-merge-and-snapshots/).
