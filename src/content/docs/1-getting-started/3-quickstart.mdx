...
When work is reviewed and a task is complete, v0.8 can create a persistent cleanup request according to project policy. Run `carryctx worktree cleanup list` to inspect pending or blocked requests and `carryctx worktree cleanup run` to retry safe reconciliation. Dirty worktrees, active sessions, locks, missing paths, and jj-colocated repositories remain visible blockers. Use `carryctx stats` and `carryctx event list` to retain audit and analytics evidence for the release.
