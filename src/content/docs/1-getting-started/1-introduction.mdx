...
CarryCtx is the intent and management layer for AI coding agents. It durably holds **what** the team intends to do — dependency-aware tasks with priorities and workflows; **who** does it — durable teams of a commander plus role-specialized subagents, with handoffs enforced by a state machine; and **what happened** — an append-only event stream, Git-aware checkpoints, immutable decision records, full-text search. Then it serves exactly the slice of that context a given caller needs, over a plain CLI or an MCP server.
...
Concretely, 0.7.0 ships no scheduler, no worker runtime, no lease or heartbeat machinery, no prompt cache, and no token optimizer. What it does ship is the durable substrate those harnesses coordinate against: a dependency-gated ready queue, claims and handoffs arbitrated safely under concurrency via compare-and-swap updates, keyset-paginated events, and read-only team projections. `carryctx team context` hands a commander an accurate picture of its team; deciding what to do with that picture, and actually dispatching the work, stays outside the CLI. Nothing in CarryCtx runs anybody's agents.
