...
A Project corresponds to one Git repository and its lifecycle contract. It's created with `carryctx init`, which writes `.carryctx/config.toml` and creates authoritative state at `<git-common-dir>/carryctx/state.sqlite`, shared by every worktree.
